...
import { singleStory, updateStory } from '../../actions/storyAction'
import { Button, Form, FormGroup, Input, Label } from 'reactstrap'
import {gameModeNumberToString} from '../../helperMethods/gameModeHelper'


const UpdateStory = ({router}) => {
	...
	const {error, success, formData, title, body, view_privacy, is_finished, collab_privacy, loading, game_mode} = values
		...
		if (router.query.slug) {
			singleStory(router.query.slug).then(data => {
				if (data.error) {
					...
					setValues({...values, title: story.title, body: story.body, view_privacy: story.view_privacy, is_finished: story.is_finished, collab_privacy: story.collab_privacy, game_mode: story.game_mode, formData: new FormData()})
					setStory(story)
					setTagsArray(story.tags)
				}
			})
		}
	}
	...
	const setTagsArray = (storyTags) => {
		let tagsArray = []
		storyTags.map((t, i) => {
			tagsArray.push(t._id)
		})
	...
	const handleVisibilityToggle = (e) => {
		...
		setValues({...values, view_privacy: invertedPrivacy})
			...
			tags && tags.map((t, i) => (
				<label key={i} className="paper-check">
					<input
						...
						type="checkbox"
						checked={findOutCheckedTag(t._id)}
					/>
					<span>{t.name}</span>
				</label>
			))
	...
	const editStory = (e) => {
		...
		updateStory(router.query.slug, formData, token).then(data => {
			if (data.error) {
				setValues({...values, error: data.error, loading: false})
			} else {
				setValues({...values, title: '', body: '', loading: false, success: `Story titled "${data.title}" is successfully updated.`})
				if (isAuthenticated() && isAuthenticated().role === 1) {
					Router.replace(`/admin`)
				} else if (isAuthenticated() && isAuthenticated().role === 0) {
					Router.replace(`/stories/${router.query.slug}`)
				}
			}
		})
	}
		...
		return (
			<Form onSubmit={editStory}>
				...
				<FormGroup>
					...
					<Input type="textarea" value={body} className="input-block" placeholder="write something..." onChange={handleChange('body')} rows="6"/>
				</FormGroup>
